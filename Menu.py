...
class Menu:
    ...
    def print_menu():
        ...
        print("3. Multiply shape with number or another shape (*)")
    ...
    def handle_choice(choice):
        try:
            if choice == "1":
                ...
                print("\nEnter first shape:")
                shape = Menu.get_valid_shape()
                other = input("Enter a number or 'shape' to multiply with: ")
                try:
                    num = float(other)
                    print(f"\nResult: {shape} * {num} = {shape * num}")
                except ValueError:
                    print("\nEnter second shape:")
                    shape2 = Menu.get_valid_shape()
                    print(f"\nResult: {shape} * {shape2} = {shape * shape2}")
